...
//       String[] servers = IJBossToolingConstants.ALL_JBOSS_SERVERS;
     ...
     public EsbModuleServerCompatTest(String serverType, String esbVersion) {
        ...
        public void setUp() {
            ...
            assertNotNull("IServer is null for serverType: " + this.serverType, server);
            module = createModule(version);
            assertNotNull("IModule is null for version: " + this.serverType, module);
        }
    ...
    public void testeServerModuleSupport() {
        assertNotNull("IServer is null", server);
        assertNotNull("Server Type is null for " + server.getId(), server.getServerType());
        assertNotNull("Server Type/Runtime Type is null for " + server.getId(), server.getServerType().getRuntimeType());
        assertNotNull("Server Type/Runtime Type/Module Types is null for " + server.getId(), server.getServerType().getRuntimeType().getModuleTypes());
        assertNotNull("IModule is null", module);
        assertNotNull("Module Type is null for " + module.getModuleType());
